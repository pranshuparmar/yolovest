...
def create_app(ctx: AppContext) -> FastAPI:
    ...
    # Auth config
    dash_password = ctx.config.dashboard.password if hasattr(ctx.config.dashboard, "password") else "yolovest"
    ...
    async def get_portfolio(user: str = Depends(verify_credentials)) -> dict:
        """Portfolio overview: capital, exposure, open positions, PnL."""
        portfolio = await ctx.db.get_portfolio_state()
        return portfolio

    @app.get("/api/positions")
    async def get_positions(user: str = Depends(verify_credentials)) -> list[dict]:
        """Current open positions."""
        return await ctx.db.get_open_positions()

    @app.get("/api/trades/today")
    async def get_todays_trades(user: str = Depends(verify_credentials)) -> list[dict]:
        """Today's trades."""
    ...
    async def get_watchlist(user: str = Depends(verify_credentials)) -> list[dict]:
        """Current watchlist with scores."""
        return await ctx.db.get_watchlist()

    @app.get("/api/sectors")
    async def get_sector_rotation(user: str = Depends(verify_credentials)) -> dict:
