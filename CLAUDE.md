...
| `strategy.swing_label_mode` | `"relative"` (default) — cross-sectional relative-momentum label: per date, forward 10-bar returns ranked across the universe; top `relative_label_quantile` → BUY, bottom → SELL. Market-drift-neutral by construction; the backtest exits at the LIVE ATR geometry (breaking barrier mode's label/exit circularity). `"barrier"` = legacy absolute triple-barrier. Intraday always uses its 1-min triple-barrier label. |
